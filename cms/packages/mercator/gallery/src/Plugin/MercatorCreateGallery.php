...
function create_all_galleries($options) {
	...
	$columns 		  = "1"; // Number of adjacent images to show when disdplaying a slider
	$finite			  = "false"; // finite slideshow or not, by default a slideshow is not finit, i.e., after the last image has been shown, it starts over with the first
	$sets			  = "false"; // When displaying a slider with multuple images ("colums" set to anything more than 1), slide all current images as opposed to one.  
			...
			foreach ($dirs as $subdir) {
				...
				switch ($mode) {
					case "accordion-lightbox":	
						...
						break;
					case "accordion-slider":
						$html .= create_gallery($images, "slider", $shuffle, $position, $width, $ratio, $animation, $columns, $autoplay, $sets, $duration, $imagesize, $thumbsize, $thumbsquare, $imagecompression, $thumbcompression);
						break;
				}
				$html .= "</div></li>";
				$opended = "";
...
function create_gallery($images, $mode, $shuffle, $position, $width, $ratio, $animation, $columns, $autoplay, $sets, $duration, $imagesize, $thumbsize, $thumbsquare, $imagecompression, $thumbcompression) {
	...
	// Shuffle images
	if ($shuffle || !strcmp($mode, "randomimage"))
